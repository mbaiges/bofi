...
async function loadChart(symbol = 'GOOGL', timeframe = '1D', options = {}) {
    try {
        // Show loading state
        ...
        // Construct API URL
        let apiUrl = `/api/candles?symbol=${encodeURIComponent(symbol)}&timeframe=${encodeURIComponent(timeframe)}`;
        ...
        if (options.from) {
            ...
            apiUrl += `&amount=${options.amount || 100}`;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('load-chart').addEventListener('click', function() {
        ...
        const options = {};
        if (fromDate) {
            options.from = Math.floor(new Date(fromDate).getTime() / 1000);
            if (toDate) {
                options.to = Math.floor(new Date(toDate).getTime() / 1000);
            } else {
                options.to = Math.floor(Date.now() / 1000);
            }
        } else {
            options.amount = amount;
        }
        
        loadChart(symbol, timeframe, options);
    });
